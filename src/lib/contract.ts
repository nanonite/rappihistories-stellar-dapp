import { MedRecord } from "@/types";

const CONTRACT_ID =
  process.env.NEXT_PUBLIC_MEDICAL_CONTRACT_ID ||
  "CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC";

export const PLACEHOLDER = true;

interface ContractRecord {
  doctor: string;
  timestamp: string;
  data_hash: string;
  record_type: string;
  notes: string;
}

function fromContractRecord(r: ContractRecord): MedRecord {
  return {
    doctor: r.doctor,
    timestamp: r.timestamp,
    dataHash: r.data_hash,
    recordType: r.record_type,
    notes: r.notes,
  };
}

export async function getPatientRecords(
  _patientAddress: string
): Promise<MedRecord[]> {
  const req = {
    jsonrpc: "2.0",
    id: 1,
    method: "simulateTransaction",
    params: {
      transaction: "AA...",
    },
  };

  const demo: ContractRecord[] = [
    {
      doctor: "GA4Q...7XYZ",
      timestamp: "1715875200",
      data_hash: "0xabc123def456",
      record_type: "lab_result",
      notes: "Hemograma: GB 7.2, GR 5.1, Hgb 14.5, Hct 42%. Todos los valores dentro del rango normal.",
    },
    {
      doctor: "GA4Q...7XYZ",
      timestamp: "1715788800",
      data_hash: "0xdef789ghi012",
      record_type: "diagnosis",
      notes: "Diagnóstico: Hipertensión esencial (I10). PA 140/90. Se prescriben modificaciones de estilo de vida y seguimiento en 3 meses.",
    },
    {
      doctor: "GB7K...9ABC",
      timestamp: "1715616000",
      data_hash: "0xghi345jkl678",
      record_type: "vaccination",
      notes: "Administrado: Refuerzo Tdap, Influenza (estacional). Sin reacciones adversas observadas post-administración.",
    },
    {
      doctor: "GB7K...9ABC",
      timestamp: "1715356800",
      data_hash: "0xjkl901mno234",
      record_type: "imaging",
      notes: "Radiografía de tórax PA: Campos pulmonares despejados bilateralmente. Sin derrame pleural. Silueta cardíaca normal.",
    },
    {
      doctor: "GA4Q...7XYZ",
      timestamp: "1715011200",
      data_hash: "0xmno567pqr890",
      record_type: "prescription",
      notes: "Rx: Lisinopril 10mg VO diario #30, reposiciones: 3. Amlodipino 5mg VO diario #30, reposiciones: 3.",
    },
  ];

  return demo.map(fromContractRecord);
}

export async function getAuthorizedDoctors(
  _patientAddress: string
): Promise<string[]> {
  const demos = [
    "GA4QBNA2K7LQDTEZBJFDPW6JBCWMNFMKW2CG7NDR4ZLXFGZBXCAJ7XYZ",
    "GB7KNYWGBOBK5J4FQY3ZP3VXXP6CMWIVFGP7NMGJTT7EES7LKAFL9ABC",
  ];
  return demos;
}

export async function isDoctorAuthorized(
  patientAddress: string,
  doctorAddress: string
): Promise<boolean> {
  const doctors = await getAuthorizedDoctors(patientAddress);
  return doctors.includes(doctorAddress);
}
